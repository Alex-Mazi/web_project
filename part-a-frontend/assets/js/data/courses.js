...
function getURLParams() {
  ...
  return {category: params.get("category"), level: params.get("level"), search: params.get("search"),};
...
function applyURLFilters() {
  ...
  if (params.category) {
    ...
    if (categoryFilter) {
      ...
      const matchingOption = options.find(
        (opt) => opt.value.toLowerCase() === params.category.toLowerCase()
      );
  ...
  if (params.level) {
    ...
    if (levelFilter) {
      ...
      const matchingOption = options.find(
        (opt) => opt.value.toLowerCase() === params.level.toLowerCase()
      );
...
function initCategories() {
  ...
  const cats = [
    ...new Set(window.courses.map((course) => course.category)),
  ].sort();
...
function filtercourses() {
  ...
  if (cat) { filtered = filtered.filter((course) => course.category === cat); }
  ...
  if (search) {
    filtered = filtered.filter((course) => {
      ...
      const matchesTopic =
        course.topics &&
        course.topics.some((t) => t.toLowerCase().includes(search));
...
function displaycourses(courses) {
  ...
  grid.innerHTML = courses.map((course) => `
    <div class="course-card2">
      <img src="${course.image}" srcset="${course.image} 320w, ${course.image} 640w, ${course.image} 960w" sizes="(max-width: 600px) 90vw, (max-width: 1024px) 45vw, 300px" alt="${course.title}" class="course-image" loading="lazy" decoding="async" onerror="this.onerror=null; this.src='assets/img/thumbnails/default-course.png'; this.srcset='';">
      <div class="course-info">
        ...
        <a href="course-details.html?id=${course.id}" class="course-link">View Course →</a>
      </div>
    </div>
  `).join("");
...
function updateResultsInfo(shown, total) {
  ...
  if (info) {
    info.textContent = shown === total? `Showing all ${total} courses`: `Showing ${shown} of ${total} courses`;
  }
